...
//    );
        ...
        @Query(value = "SELECT r.* FROM ride_requests r " +
                "WHERE r.status = 'PENDING' " +
                // 🟢 FIX 1: LOWER + CONCAT % allows partial sub-string fuzzy matches for address fragments
                "AND LOWER(r.source) LIKE LOWER(CONCAT('%', :source, '%')) " +
                "AND LOWER(r.destination) LIKE LOWER(CONCAT('%', :destination, '%')) " +
                // 🟢 FIX 2: Handle fallback if driver sets filter to 'ANY'
                "AND (:preferredVehicle = 'ANY' OR r.preferred_vehicle = :preferredVehicle) " +
                "AND CONCAT(TO_CHAR(r.preferred_time, 'YYYY-MM-DD HH24:'), " +
                "   (CASE WHEN EXTRACT(MINUTE FROM r.preferred_time) < 30 THEN '00' ELSE '30' END)) = :timeSlot " +
